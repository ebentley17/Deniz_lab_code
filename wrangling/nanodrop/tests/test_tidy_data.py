"""Tests for all the subfunctions in tidy_data. 
...
def test_clean_up_columns():
    ...
    cleaned = td.clean_up_columns(messy_df)
    ...
    cleaned_again = td.clean_up_columns(cleaned)
    assert (cleaned_again == cleaned).all().all()


def test_rename_abs_columns_by_wavelength_simple_case():
    renamed = td.rename_abs_columns_by_wavelength(make_test_data())
...
def test_rename_abs_columns_by_wavelength():
    ...
    renamed = td.rename_abs_columns_by_wavelength(test_df)
...
def test_ParseKey():
    # TODO
    test_key = td.ParseKey(("hi", str), separator=" ")
    test_key.separator = " "
    test_key.parse_key = ("hi", str)
    test_key.column_names = ("hi",)
    
    with pytest.raises(RuntimeError):
        td.ParseKey(10, separator=" ")
        
    with pytest.raises(RuntimeError):
        td.ParseKey(("10", "hi", str), separator="")  

    with pytest.raises(RuntimeError):
        td.ParseKey((str, "type"), separator=" ")
    
    with pytest.raises(RuntimeError):
        td.ParseKey(("str", "type"), separator=" ")
        
    with pytest.raises(RuntimeError):
        td.ParseKey(("pep", str), ("pep", float), separator=" ")

    
def test_make_columns_by_parse_key():
    test_data = make_test_data()
    df_new_cols = td._make_columns_by_parse_key(test_data, ParseKey=td.parse_rna_peptide)

    assert test_data.columns.isin(df_new_cols.columns).all()
    for key, _ in td.parse_rna_peptide.parse_key:
        assert key in df_new_cols.columns

    with pytest.warns(UserWarning):
        bad_parse_key = td.ParseKey(("This column fails", list), separator="_")
        td._make_columns_by_parse_key(test_data, bad_parse_key)


def test_identify_buffer_measurements():
    test_df = make_test_data()
    assert (td._identify_buffer_measurements(test_df) == [1]).all()

    test_df.loc[2, "Sample ID"] = "Buffer_100_0.5"
    assert (td._identify_buffer_measurements(test_df) == [1, 2]).all()

    test_df.loc[:, "Sample ID"] = "Peptide_100_0.5"
    assert len(td._identify_buffer_measurements(test_df)) == 0


def test_handle_incorrectly_named_samples_drop_all():
    test_df = make_test_data()
    dropped = td._handle_incorrectly_named_samples(
        test_df, [0], drop_incorrectly_named_samples=True, drop_buffers=True
    )
    assert len(dropped) == 1
    assert len(td._identify_buffer_measurements(dropped)) == 0


def test_handle_incorrectly_named_samples_drop_none():
    test_df = make_test_data()
    with pytest.warns(UserWarning):
        not_dropped = td._handle_incorrectly_named_samples(
...
def test_handle_incorrectly_named_samples_keep_buffers_only():
    ...
    assert len(dropped) == 2
    assert len(td._identify_buffer_measurements(dropped)) == 1


def test_analyze_sample_names_buffer_dropping_only():
    test_df = make_test_data()
    dropped = td.analyze_sample_names(test_df, drop_buffers=True)
    assert len(dropped) == 2
    assert len(td._identify_buffer_measurements(dropped)) == 0

    not_dropped = td.analyze_sample_names(test_df, drop_buffers=False)
    assert len(not_dropped) == 3
    assert len(td._identify_buffer_measurements(not_dropped)) == 1
...
def test_analyze_sample_names_drop_incorrectly_named_samples():
    ...
    dropped_both = td.analyze_sample_names(
        test_df, drop_incorrectly_named_samples=True, drop_buffers=True
    )
    assert len(dropped_both) == 1
    assert len(td._identify_buffer_measurements(dropped_both)) == 0

    dropped_incorrect_not_buffers = td.analyze_sample_names(
        test_df, drop_incorrectly_named_samples=True, drop_buffers=False
    )
    assert len(dropped_incorrect_not_buffers) == 2
    assert len(td._identify_buffer_measurements(dropped_incorrect_not_buffers)) == 1

    with pytest.warns(UserWarning) as sample_name_divergence:
        td.analyze_sample_names(test_df, drop_incorrectly_named_samples=False)
    assert (
...
def test_analyze_sample_names_new_columns():
    analyzed = td.analyze_sample_names(make_test_data())
...
def test_analyze_sample_names_new_columns_kDNA_parse_key():
    analyzed = td.analyze_sample_names(make_test_data(), ParseKey=td.parse_kdna_mg2)
...
def test_analyze_sample_names_contents():
    ...
    analyzed = td.analyze_sample_names(test_data)
...
def test_break_out_date_and_time():
    ...
    broken_out = td.break_out_date_and_time(test_df)
    new_cols = ["Date", "Time"]
...
def test_drop_zeros():
    ...
    dropped = td.drop_zeros(test_df, "1 (nm)")
    assert len(dropped) == 2
...
def test_drop_zeros_several_columns():
    ...
    dropped = td.drop_zeros(test_df, ["1 (nm)", "2 (nm)"])
    assert len(dropped) == 1
    assert 0 not in dropped["1 (nm)"].values
