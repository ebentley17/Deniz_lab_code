"""Tests for all the subfunctions in nanodrop.py 
Tests for the full tidy_data function are sequestered in test_tidy_data."""
...
def test_clean_up_columns():
    ...
    cleaned = nd.clean_up_columns(messy_df)
    ...
    cleaned_again = nd.clean_up_columns(cleaned)
    assert (cleaned_again == cleaned).all().all()


def test_rename_abs_columns_by_wavelength_simple_case():
    renamed = nd.rename_abs_columns_by_wavelength(make_test_data())
...
def test_rename_abs_columns_by_wavelength():
    ...
    renamed = nd.rename_abs_columns_by_wavelength(test_df)
...
def test_ParseKey():
    # TODO
    test_key = nd.ParseKey(("hi", str), separator=" ")
    test_key.separator = " "
    test_key.parse_key = ("hi", str)
    test_key.column_names = ("hi",)
    
    with pytest.raises(RuntimeError):
        nd.ParseKey(10, separator=" ")
        
    with pytest.raises(RuntimeError):
        nd.ParseKey(("10", "hi", str), separator="")  

    with pytest.raises(RuntimeError):
        nd.ParseKey((str, "type"), separator=" ")
    
    with pytest.raises(RuntimeError):
        nd.ParseKey(("str", "type"), separator=" ")
        
    with pytest.raises(RuntimeError):
        nd.ParseKey(("pep", str), ("pep", float), separator=" ")

    
def test_make_columns_by_parse_key():
    test_data = make_test_data()
    df_new_cols = nd._make_columns_by_parse_key(test_data, ParseKey=nd.parse_rna_peptide)

    assert test_data.columns.isin(df_new_cols.columns).all()
    for key, _ in nd.parse_rna_peptide.parse_key:
        assert key in df_new_cols.columns

    with pytest.warns(UserWarning):
        bad_parse_key = nd.ParseKey(("This column fails", list), separator="_")
        nd._make_columns_by_parse_key(test_data, bad_parse_key)
...
def test_handle_incorrectly_named_samples_drop_none():
    test_df = make_test_data()
    with pytest.warns(UserWarning):
        not_dropped = nd._handle_incorrectly_named_samples(
...
def test_analyze_sample_names_drop_incorrectly_named_samples():
    ...
    dropped_both = nd.analyze_sample_names(
    ...
    assert len(nd._identify_buffer_measurements(dropped_incorrect_not_buffers)) == 1
...
def test_analyze_sample_names_new_columns():
    analyzed = nd.analyze_sample_names(make_test_data())
...
#     analyzed = nd.analyze_sample_names(make_test_data(), ParseKey=nd.parse_kdna_mg2)
...
def test_analyze_sample_names_contents():
    ...
    analyzed = nd.analyze_sample_names(test_data)
...
def test_break_out_date_and_time():
    ...
    broken_out = nd.break_out_date_and_time(test_df)
    new_cols = ["Date", "Time"]
...
def test_drop_zeros():
    ...
    dropped = nd.drop_zeros(test_df, "1 (nm)")
    assert len(dropped) == 2
...
def test_drop_zeros_several_columns():
    ...
    dropped = nd.drop_zeros(test_df, ["1 (nm)", "2 (nm)"])
    assert len(dropped) == 1
    assert 0 not in dropped["1 (nm)"].values
